...
type CabinetPayload = { upcoming?: Array<ClientBooking & { booking_code?: string }>; past?: Array<ClientBooking & { booking_code?: string }> };
...
const stripInternalReference = ({ booking_code: _bookingCode, ...booking }: ClientBooking & { booking_code?: string }): ClientBooking => booking;
...
export const clientAccountApi = {
  ...
  async bookings(): Promise<ClientBooking[]> {
    ...
    const upcoming = normalizeList<ClientBooking & { booking_code?: string }>(payload.upcoming, ['upcoming']);
    const past = normalizeList<ClientBooking & { booking_code?: string }>(payload.past, ['past']);
    const list = [...upcoming, ...past].map(stripInternalReference);
