...
export const businessApi = {
  async login(email: string, password: string) { const { data } = await businessAuthClient.post('/auth/login', { email, password }); assertBusinessAudience(data); await tokenStore.set('business', data.token); void synchronizePushDevice('business'); return data.user as BusinessUser; },
  async register(payload: BusinessRegistration) { const { data } = await businessAuthClient.post('/auth/register', payload); assertBusinessAudience(data); await tokenStore.set('business', data.token); void synchronizePushDevice('business'); return data.user as BusinessUser; },
  async me() { const { data } = await businessAuthClient.get('/auth/me'); return normalizeResource<BusinessUser>(data, ['user']); },
  async dashboard() { const { data } = await businessAuthClient.get('/dashboard'); return normalizeResource(data); },
  ...
  async createClient(payload: { name: string; phone?: string; email?: string }) { const { data } = await businessAuthClient.post('/clients', payload); return normalizeResource<BusinessClient>(data, ['client']); },
  ...
  async updateClient(id: number, payload: Partial<BusinessClientDetail>) { const { data } = await businessAuthClient.put(`/clients/${id}`, payload); return normalizeResource<BusinessClientDetail>(data, ['client']); },
  ...
  async createTask(payload: { title: string; description?: string; priority?: BusinessTask['priority']; assignee_id?: number }) { const { data } = await businessAuthClient.post('/tasks', payload); return normalizeResource<BusinessTask>(data, ['task']); },
  async updateTask(id: number, payload: Partial<BusinessTask>) { const { data } = await businessAuthClient.patch(`/tasks/${id}`, payload); return normalizeResource<BusinessTask>(data, ['task']); },
  async deleteTask(id: number) { await businessAuthClient.delete(`/tasks/${id}`); },
  async analytics() { const { data } = await businessAuthClient.get('/analytics/overview'); return normalizeResource(data); },
  ...
  async createGiftCard(payload: { amount: number; issued_to_name?: string; issued_to_phone?: string }) { const { data } = await businessAuthClient.post('/gift-cards', { ...payload, currency: 'AMD' }); return normalizeResource<GiftCard>(data, ['gift_card']); },
  async loyalty() { const { data } = await businessAuthClient.get('/loyalty/summary'); return normalizeResource(data); },
  async loyaltyProgram() { const { data } = await businessAuthClient.get('/loyalty/program'); return normalizeResource(data); },
  async updateLoyaltyProgram(payload: Record<string, unknown>) { const { data } = await businessAuthClient.put('/loyalty/program', payload); return normalizeResource(data); },
  ...
  async updateWaitlist(id: number, status: 'waiting' | 'cancelled') { const { data } = await businessAuthClient.patch(`/waitlist/${id}`, { status }); return normalizeResource(data); },
  async billing() { const { data } = await businessAuthClient.get('/billing/me'); return normalizeResource(data); },
  async telegram(): Promise<{ available: boolean; connected: boolean; bot_url?: string | null }> { const { data } = await businessAuthClient.get('/telegram/connection'); return normalizeResource(data); },
  async createTelegramLink(): Promise<{ url: string }> { const { data } = await businessAuthClient.post('/telegram/connection'); return normalizeResource(data); },
  async disconnectTelegram() { await businessAuthClient.delete('/telegram/connection'); },
  async settings() { const { data } = await businessAuthClient.get('/business/settings'); return normalizeResource(data); },
  async updateSettings(payload: Record<string, unknown>) { const { data } = await businessAuthClient.patch('/business/settings', payload); return normalizeResource(data); },
  async uploadImage(file: LocalImageFile, folder = 'businesses') { const form = new FormData(); form.append('file', { uri: file.uri, name: file.name, type: file.mimeType } as unknown as Blob); form.append('folder', folder); const { data } = await businessAuthClient.post('/media/upload', form); return normalizeResource<{ path: string; url: string }>(data); },
  async createLocation(payload: Omit<BusinessLocation, 'id' | 'is_active'> & { is_active?: boolean }) { const { data } = await businessAuthClient.post('/business/locations', payload); return normalizeResource<BusinessLocation>(data, ['location']); },
  async updateLocation(id: number, payload: Partial<BusinessLocation>) { const { data } = await businessAuthClient.patch(`/business/locations/${id}`, payload); return normalizeResource<BusinessLocation>(data, ['location']); },
  async deleteLocation(id: number) { const { data } = await businessAuthClient.delete(`/business/locations/${id}`); return normalizeResource(data); },
  async onboardingStatus() { const { data } = await businessAuthClient.get('/business/onboarding-status'); return normalizeResource(data); },
  async createService(payload: { name: string; duration_minutes: number; price?: number; currency?: string; location_id?: number }) { const { data } = await businessAuthClient.post('/services', { ...payload, is_active: true }); return normalizeResource<BusinessService>(data, ['service']); },
  async updateService(id: number, payload: Partial<BusinessService>) { const { data } = await businessAuthClient.put(`/services/${id}`, payload); return normalizeResource<BusinessService>(data, ['service']); },
  async deleteService(id: number) { await businessAuthClient.delete(`/services/${id}`); },
  async createStaff(payload: { name: string; email: string; password: string; phone?: string; location_id?: number }) { const { data } = await businessAuthClient.post('/staff', { ...payload, role: 'staff', is_bookable: true, show_in_public_team: true }); return normalizeResource<BusinessStaff>(data, ['staff', 'user']); },
  async updateStaff(id: number, payload: Partial<BusinessStaff>) { const { data } = await businessAuthClient.patch(`/staff/${id}`, payload); return normalizeResource<BusinessStaff>(data, ['staff', 'user']); },
  ...
  async updateStaffSchedule(id: number, days: ScheduleDay[]) { const { data } = await businessAuthClient.put(`/staff/${id}/schedule`, { days }); return normalizeResource(data); },
  async completeOnboarding() { return (await businessAuthClient.post('/business/complete-onboarding')).data; },
  async createBooking(payload: { service_id: number; staff_id: number; location_id?: number; starts_at: string; client_name: string; client_phone: string; client_email?: string; client_id?: number; notes?: string }) { const { data } = await businessAuthClient.post('/bookings', { ...payload, status: 'confirmed', source: 'admin' }); return normalizeResource(data); },
  async updateStatus(id: number, status: 'confirm' | 'done' | 'no-show' | 'cancel') { const { data } = await businessAuthClient.patch(`/bookings/${id}/${status}`); return normalizeResource(data); },
