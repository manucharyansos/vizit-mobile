...
export const tokenStore = { get: (audience: TokenAudience) => SecureStore.getItemAsync(keys[audience]), set: (audience: TokenAudience, token: string) => SecureStore.setItemAsync(keys[audience], token), remove: (audience: TokenAudience) => SecureStore.deleteItemAsync(keys[audience]) };
export function createApiClient(audience?: TokenAudience) {
  ...
  client.interceptors.request.use(async (config) => { if (audience) { const token = await tokenStore.get(audience); if (token) config.headers.Authorization = `Bearer ${token}`; } config.headers['X-Mobile-Client'] = 'vizit-mobile'; return config; });
  client.interceptors.response.use((response) => response, async (error) => { if (audience && error?.response?.status === 401) await tokenStore.remove(audience); return Promise.reject(error); });
  return client;
}
