...
export default function BookingScreen() {
  ...
  const visibleSlots = useMemo(
    () => (slots.data ?? []).filter((item) => !staff || item.staff_id === staff.id),
    [slots.data, staff],
  );
  ...
  return <SafeAreaView style={[styles.screen, { backgroundColor: theme.background }]}>
    ...
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      {service ? <Section step="3" title={t('chooseDate')}><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontal}>{dates.map((item) => <DateChoice key={item} value={item} selected={date === item} locale={locale} onPress={() => { setDate(item); setSlot(undefined); }} />)}</ScrollView><Text style={[styles.timeTitle, { color: theme.text }]}>{t('chooseTime')}</Text>{slots.isFetching && !slots.data ? <ActivityIndicator color={theme.plum} /> : slots.isError ? <ErrorState onRetry={() => slots.refetch()} /> : !visibleSlots.length ? <Text style={[styles.emptyText, { color: theme.muted }]}>{c.noSlots}</Text> : <View style={styles.slotGrid}>{visibleSlots.map((item) => <Choice key={`${item.starts_at}-${item.staff_id}`} compact recommended={Boolean(item.is_recommended)} selected={slot?.starts_at === item.starts_at && slot.staff_id === item.staff_id} label={`${item.is_recommended ? '★ ' : ''}${formatApiTime(item.starts_at, locale)}–${formatApiTime(item.ends_at, locale)}`} detail={!staff ? item.staff_name : item.is_recommended ? t('recommended') : undefined} onPress={() => setSlot(item)} />)}</View>}</Section> : null}
