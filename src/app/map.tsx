      ...
      {selected ? <View style={[styles.businessCard, { backgroundColor: theme.surfaceRaised, borderColor: theme.border, shadowColor: theme.shadow }]}>
        <View style={styles.businessCardTop}>
          <View style={styles.businessText}>
            ...
            {selected.business.category_name ? <Text numberOfLines={1} style={[styles.businessMeta, { color: theme.muted }]}>{selected.business.category_name}</Text> : null}
        ...
        <View style={styles.actions}>
          <Pressable onPress={() => router.push({ pathname: '/business/[slug]', params: { slug: selected.business.slug } })} style={[styles.secondary, { borderColor: theme.border }]}><Text style={{ color: theme.text, fontWeight: '900' }}>{details}</Text></Pressable>
          <Pressable onPress={() => router.push({ pathname: '/book/[slug]', params: { slug: selected.business.slug, ...(selectedLocation?.id ? { locationId: String(selectedLocation.id) } : {}) } })} style={[styles.primary, { backgroundColor: theme.plum }]}><Text style={styles.primaryText}>{book}</Text><VizitIcon ios="calendar.badge.plus" android="event_available" color="#FFFFFF" size={18} /></Pressable>
        </View>
      </View> : null}
    </View>}
  </SafeAreaView>;
