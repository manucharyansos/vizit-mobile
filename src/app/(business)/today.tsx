...
export default function TodayScreen() {
  ...
  const status = useMutation({
    ...
    onError: () => Alert.alert(c.loadError),
  });
...
function BookingCard({ item, onStatus, labels, locale }: { item: CalendarBooking; onStatus: (action: "confirm" | "done" | "no-show" | "cancel") => void; labels: typeof copy.hy; locale: 'hy' | 'ru' | 'en' }) {
  ...
  return <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
    ...
    <Text style={[styles.client, { color: theme.text }]}>{item.client_name ?? item.customer_name ?? labels.client}</Text>
...
function Action({ title, onPress, tone }: { title: string; onPress: () => void; tone?: "primary" | "success" | "danger" }) {
  const { theme } = useApp();
  const background = tone === "primary" ? theme.plumSoft : tone === "success" ? theme.goldSoft : tone === "danger" ? theme.dangerSoft : theme.background;
...
const styles = StyleSheet.create({
  screen: { flex: 1 }, center: { flex: 1, alignItems: "center", justifyContent: "center", padding: 24, gap: 18 },
  header: { paddingHorizontal: 20, paddingTop: 12, paddingBottom: 18, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 12 },
  eyebrow: { fontSize: 9, fontWeight: "900", letterSpacing: 1.5, marginBottom: 4 }, title: { fontSize: 27, fontWeight: "900", letterSpacing: -0.5 },
  logout: { width: 43, height: 43, borderRadius: 9, alignItems: "center", justifyContent: "center" }, dateCard: { marginHorizontal: 18, padding: 13, borderRadius: 10, flexDirection: "row", alignItems: "center", gap: 11 },
  dateIcon: { width: 42, height: 42, borderRadius: 10, alignItems: "center", justifyContent: "center" }, dateText: { flex: 1, fontSize: 14, fontWeight: "800", textTransform: "capitalize" }, countBadge: { width: 30, height: 30, borderRadius: 15, alignItems: "center", justifyContent: "center" }, countText: { color: "#FFF", fontWeight: "900", fontSize: 12 },
  list: { padding: 18, gap: 12 }, emptyWrap: { alignItems: "center", paddingTop: 70, gap: 12 }, empty: { textAlign: "center" }, card: { padding: 14, borderRadius: 11, borderWidth: 1, gap: 8 },
  cardTop: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, timeBadge: { paddingHorizontal: 11, height: 35, borderRadius: 9, flexDirection: "row", alignItems: "baseline" }, time: { fontSize: 18, fontWeight: "900" }, timeEnd: { fontSize: 12, fontWeight: "700", marginLeft: 3 },
  statusBadge: { paddingHorizontal: 9, paddingVertical: 6, borderRadius: 9 }, status: { fontWeight: "900", textTransform: "uppercase", fontSize: 9 }, client: { fontSize: 17, fontWeight: "900", marginTop: 2 }, detailRow: { flexDirection: "row", alignItems: "center", gap: 6 },
  actions: { flexDirection: "row", gap: 5, marginTop: 8 }, action: { flex: 1, minHeight: 39, borderRadius: 8, alignItems: "center", justifyContent: "center", padding: 4 }, primary: { paddingHorizontal: 18, paddingVertical: 13, borderRadius: 9 }, white: { color: "#FFF", fontWeight: "800" },
  error: { margin: 18, borderWidth: 1, borderRadius: 9, padding: 14, gap: 8 },
});