...
import { businessApi, BusinessLocation, BusinessService } from '@/services/api/business';
import { apiErrorMessage } from '@/services/api/client';

const copy = {
  hy: { title: 'Ծառայություններ', empty: 'Ծառայություններ դեռ չկան', add: 'Նոր ծառայություն', name: 'Անվանում', duration: 'Տևողություն՝ րոպե', price: 'Գին՝ ֏', save: 'Պահպանել', cancel: 'Չեղարկել', active: 'Ակտիվ', edit: 'Խմբագրել', remove: 'Ջնջել', confirm: 'Ջնջե՞լ այս ծառայությունը', error: 'Չհաջողվեց պահպանել', location: 'Մասնաճյուղ', loadError: 'Չհաջողվեց բեռնել ծառայությունները', retry: 'Կրկին փորձել' },
  ru: { title: 'Услуги', empty: 'Услуг пока нет', add: 'Новая услуга', name: 'Название', duration: 'Длительность, минуты', price: 'Цена, ֏', save: 'Сохранить', cancel: 'Отмена', active: 'Активна', edit: 'Изменить', remove: 'Удалить', confirm: 'Удалить эту услугу?', error: 'Не удалось сохранить', location: 'Филиал', loadError: 'Не удалось загрузить услуги', retry: 'Повторить' },
  en: { title: 'Services', empty: 'No services yet', add: 'New service', name: 'Name', duration: 'Duration, minutes', price: 'Price, ֏', save: 'Save', cancel: 'Cancel', active: 'Active', edit: 'Edit', remove: 'Delete', confirm: 'Delete this service?', error: 'Could not save', location: 'Location', loadError: 'Could not load services', retry: 'Try again' },
};

type ServiceForm = { name: string; duration: string; price: string; locationId?: number };
const blank = (locationId?: number): ServiceForm => ({ name: '', duration: '60', price: '', locationId });
...
export default function ServicesScreen() {
  ...
  const save = useMutation({
    mutationFn: () => editing
      ? businessApi.updateService(editing.id, { name: form.name.trim(), duration_minutes: Number(form.duration), price: Number(form.price || 0), location_id: form.locationId })
      : businessApi.createService({ name: form.name.trim(), duration_minutes: Number(form.duration), price: Number(form.price || 0), currency: 'AMD', location_id: form.locationId }),
  ...
  const beginEdit = (item: BusinessService) => { setEditing(item); setForm({ name: item.name, duration: String(item.duration_minutes), price: String(item.price ?? 0), locationId: item.location_id ?? defaultLocationId }); };
