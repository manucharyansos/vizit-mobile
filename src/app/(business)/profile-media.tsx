...
const copy = {
  hy: { title: 'Լոգո և նկարներ', hint: 'Այս նկարները ցուցադրվում են Vizit-ի հրապարակային էջում և որոնման արդյունքներում։', logo: 'Բիզնեսի լոգո', cover: 'Գլխավոր շապիկ', choose: 'Ընտրել gallery-ից', remove: 'Հեռացնել', permission: 'Նկար ընտրելու համար թույլատրեք gallery-ի հասանելիությունը։', saved: 'Նկարը պահպանված է', failed: 'Նկարը չհաջողվեց բեռնել' },
  ru: { title: 'Логотип и фото', hint: 'Эти изображения показываются на публичной странице Vizit и в результатах поиска.', logo: 'Логотип бизнеса', cover: 'Главная обложка', choose: 'Выбрать из галереи', remove: 'Удалить', permission: 'Разрешите доступ к галерее, чтобы выбрать изображение.', saved: 'Изображение сохранено', failed: 'Не удалось загрузить изображение' },
  en: { title: 'Logo and images', hint: 'These images appear on your public Vizit page and in search results.', logo: 'Business logo', cover: 'Main cover', choose: 'Choose from gallery', remove: 'Remove', permission: 'Allow photo library access to choose an image.', saved: 'Image saved', failed: 'Could not upload image' },
};
export default function ProfileMedia() {
  const { locale, theme } = useApp(); const c = copy[locale]; const qc = useQueryClient();
  const settings = useQuery<Settings>({ queryKey: ['business-settings'], queryFn: businessApi.settings });
  const update = useMutation({ mutationFn: ({ key, value }: { key: 'logo_url' | 'cover_url'; value: string | null }) => businessApi.updateSettings({ [key]: value }), onSuccess: () => { void qc.invalidateQueries({ queryKey: ['business-settings'] }); Alert.alert(c.saved); }, onError: (e) => Alert.alert(c.failed, apiErrorMessage(e)) });
...
function MediaCard({ title, uri, shape, choose, remove, busy, onChoose, onRemove }: { title: string; uri?: string | null; shape: 'logo' | 'cover'; choose: string; remove: string; busy: boolean; onChoose: () => void; onRemove: () => void }) {
  const { theme } = useApp(); return <View style={[styles.card, { backgroundColor: theme.surfaceRaised, borderColor: theme.border }]}><Text style={[styles.cardTitle, { color: theme.text }]}>{title}</Text><View style={[shape === 'logo' ? styles.logoPreview : styles.coverPreview, { backgroundColor: theme.plumSoft }]}>{uri ? <Image source={{ uri }} style={StyleSheet.absoluteFill} contentFit="cover" transition={180} /> : <VizitIcon ios="photo" android="image" color={theme.plum} size={32} />}</View><Pressable disabled={busy} onPress={onChoose} style={[styles.primary, { backgroundColor: theme.plum }]}>{busy ? <ActivityIndicator color="#FFF" /> : <><VizitIcon ios="photo.on.rectangle" android="photo_library" color="#FFF" size={19} /><Text style={styles.white}>{choose}</Text></>}</Pressable>{uri && <Pressable disabled={busy} onPress={onRemove} style={[styles.remove, { borderColor: theme.border }]}><Text style={{ color: theme.danger, fontWeight: '800' }}>{remove}</Text></Pressable>}</View>;
}
const styles = StyleSheet.create({ screen: { flex: 1 }, center: { flex: 1, alignItems: 'center', justifyContent: 'center' }, content: { padding: 18, paddingBottom: 44, gap: 13 }, header: { flexDirection: 'row', alignItems: 'center', gap: 12 }, back: { width: 43, height: 43, borderWidth: 1, borderRadius: 9, alignItems: 'center', justifyContent: 'center' }, title: { flex: 1, fontSize: 25, fontWeight: '900' }, hint: { fontSize: 13, lineHeight: 19 }, card: { borderWidth: 1, borderRadius: 10, padding: 14, gap: 11 }, cardTitle: { fontSize: 17, fontWeight: '900' }, logoPreview: { width: 120, height: 120, borderRadius: 10, overflow: 'hidden', alignSelf: 'center', alignItems: 'center', justifyContent: 'center' }, coverPreview: { width: '100%', aspectRatio: 16 / 9, borderRadius: 9, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }, primary: { height: 52, borderRadius: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, white: { color: '#FFF', fontWeight: '900' }, remove: { height: 48, borderWidth: 1, borderRadius: 8, alignItems: 'center', justifyContent: 'center' } });
